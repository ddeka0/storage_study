...
int direct_to_disk = O_DIRECT | O_SYNC;
 ...
 */
template<typename T>
union number_template {
    T n;
    char d[sizeof(T)];
};


using number = number_template<int>;
...
void print_data(int fd, int l, int r) {
    ...
    while (total_bytes_to_read > 0) {
        ...
        for (const auto& k : std::span(x,chunk)) {
            LOG(INFO) << k.n;
        }
    }
#endif
...
void check_sorted(int fd, int l, int r) {
    CHECK_LT(l, r);
    int total_bytes_to_read = r - l;
    int offset = l;
    int buffer_size = chunk * sizeof(number::n);
 ...
 * @param chunkSize
 */
int ext_merge(int in_fd, int out_fd, int left, int right, int buffer_size) {
    CHECK_LE(left, right);
    int sz = right - left;
    // if the sz is already sorted no need to proceed
    if (sz <= buffer_size) return in_fd;
    ...
    #ifdef V2
    if (((sz / buffer_size) * 1LL) & 0x55555555) {
        std::swap(in_fd, out_fd);
    }
    #endif
    ...
    print_data(out_fd, left, right);
    check_sorted(out_fd,left, right);
...
int main(int argc, char* argv[]) {
    ...
    // this alignment is required if we want to use O_DIRECT | O_SYNC
    x = static_cast<number*>(aligned_alloc(4096,chunk * sizeof(number::n)));
    y = static_cast<number*>(aligned_alloc(4096,chunk * sizeof(number::n)));
    z = static_cast<number*>(aligned_alloc(4096,chunk * sizeof(number::n)));
    ...
    if(in_fd == ext_merge(in_fd, out_fd, 0, sz, chunk * sizeof(number::n))) {
        LOG(INFO) <<"Sorted file is numbers.txt"; 
    }else {
        LOG(INFO) <<"Sorted file is sorted_numbers.txt";
    }
